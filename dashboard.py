...
class Dashboard:
    ...
    def draw(self, frames, counts, lights, remaining, phase, current, green_times):
        ...
        for i in range(4):
            ...
            # Status
            cv2.putText(dashboard, f"S{i+1}", (x + 50, y), FONT, 0.6, WHITE, 2)
            cv2.putText(dashboard, f"Queue : {counts[i]}", (x + 50, y + 25), FONT, 0.5, WHITE, 1)
            cv2.putText(dashboard, f"Green : {green_times[i]}s", (x + 200, y + 25), FONT, 0.5, WHITE, 1)
