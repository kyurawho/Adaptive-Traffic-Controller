...
class TrafficController:
    ...
    def vehicle_entered(self, intersection):
        ...
        if (intersection != self.current or self.phase != "GREEN"):
            self.vehicle_count[intersection] += 1

    # =====================================================
    # RESET QUEUE
    # =====================================================
    def reset_queue(self, intersection):
        self.vehicle_count[intersection] = 0
    ...
    def update(self):
        ...
        if self.phase == "GREEN":
            if self.remaining <= 0:
                self.phase = "YELLOW"
                self.lights[self.current] = "YELLOW"
        ...
        # =================================================
            ...
            if self.remaining <= 0:
                ...
                # =========================================
                self.reset_queue(self.current)

                # =========================================
                # HITUNG GREEN TIME
                # =========================================
                queue = self.vehicle_count[self.current]
                self.green_time[self.current] = self.calculate_green(queue)
                ...
                # SIMPANG BARU HIJAU
                # =========================================
                self.lights[self.current] = "GREEN"
                self.phase = "GREEN"
                self.remaining = (
                self.green_time[self.current])
